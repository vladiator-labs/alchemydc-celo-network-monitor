...
import {
  discordAddressDetails,
  addressExplorerUrl,
  discordSignerDowntime,
  discordSignerBlocksProposed,
} from "./alert";
...
export default class MonitorValidators extends MonitorBase {
  protected async run() {
    ...
    const validatorSetSize = await election.numberValidatorsInSet(
      this.blocks[this.blocks.length - 1].number
    );

    console.debug(
      "There are " + validatorSetSize + " validators in the elected set"
    );
  ...
  async alertOnMissedBlocks(
    ...
    const expectedBlockCount = Math.floor(
      signatures.eligibleBlocks / validatorSetSize
    );
    ...
    console.debug(
      `monitorValidator::alertOnMissedBlocks: blocks.length ${blocks.length} proposedBlockCount ${proposedBlockCount}, expectedBlockCount ${expectedBlockCount}, missedBlockCount ${missedBlockCount}`
    );
    ...
    if (missedBlockCount > 1) {
      ...
      if (!this.doBlocksIncludeEpochTransition()) {
        await this.alert.discordWarn(
          ...
          `${this.addresses.alias(
            validator
          )} is failing to propose blocks as often as expected`
        );
      }
    }
            ...
            if (
              ...
              await this.alert.discordError(
                `\`${this.addresses.alias(
                  ...
                  block.number
                }\` and is considered offline ${discordSignerDowntime(signer)}`,
                30 * 60,
