...
export default class MonitorGovernance extends MonitorBase {
  ...
  async monitorProposal(governance: GovernanceWrapper, id: BigNumber) {
    ...
    if (
      ...
      console.debug(
        `proposal referendum time (local TZ): ${getLocalTimeString(
          referendum,
      ...
      console.debug(
        `proposal execution time (local TZ): ${getLocalTimeString(
          execution,
          "MMM Do YYYY, HH:mm zz"
        )}`
      );
      ...
      console.debug(
        `Discord alert key: ${stage}-${this.isInPast(
          referendum
        )}-${this.isInPast(execution)}-proposal-${id}`
      );
      ...
      if (stage.includes("referendum")) {
        ...
        console.debug(
          `Discord alert key: ${stage}-${this.isInPast(
            referendum
          )}-${this.isInPast(execution)}-proposal-${id}-votes`
        );
        await this.alert.discord(
          msg2,
          24 * 60 * 60,
